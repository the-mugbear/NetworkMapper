...
from app.db.models_v2 import Host, PortV2, ScriptV2, HostScriptV2
...
def get_hosts_v2(
    ...
    query = db.query(Host).options(
        selectinload(Host.ports).selectinload(PortV2.scripts),
        selectinload(Host.host_scripts)
    )
    
    # Apply filters
    if state:
        query = query.filter(Host.state == state)
    
    if os_filter:
        query = query.filter(
            or_(
                Host.os_name.ilike(f'%{os_filter}%'),
                Host.os_family.ilike(f'%{os_filter}%')
            )
        )
    ...
    if ports or services or port_states or has_open_ports:
        ...
        port_subquery = db.query(Host.id).join(PortV2)
        
        if ports:
            port_list = [int(p.strip()) for p in ports.split(',') if p.strip().isdigit()]
            if port_list:
                port_subquery = port_subquery.filter(PortV2.port_number.in_(port_list))
        
        if services:
            service_list = [s.strip().lower() for s in services.split(',') if s.strip()]
            if service_list:
                service_conditions = [PortV2.service_name.ilike(f'%{service}%') for service in service_list]
                port_subquery = port_subquery.filter(or_(*service_conditions))
        
        if port_states:
            state_list = [s.strip().lower() for s in port_states.split(',') if s.strip()]
            if state_list:
                port_subquery = port_subquery.filter(PortV2.state.in_(state_list))
        
        if has_open_ports is not None:
            if has_open_ports:
                port_subquery = port_subquery.filter(PortV2.state == 'open')
            else:
                # Get hosts that don't have any open ports
                open_host_ids = db.query(Host.id).join(PortV2).filter(PortV2.state == 'open')
                query = query.filter(~Host.id.in_(open_host_ids))
                port_subquery = None  # Skip port subquery filter
        
        if port_subquery:
            query = query.filter(Host.id.in_(port_subquery))
    ...
    if search:
        host_search_conditions = [
            ...
            Host.os_name.contains(search),
            Host.os_family.contains(search)
        ]
        ...
        if search.isdigit():
            port_search_conditions.append(PortV2.port_number == int(search))
        
        service_ports = SERVICE_PORT_MAPPINGS.get(search_lower)
        if service_ports:
            port_search_conditions.append(PortV2.port_number.in_(service_ports))
        
        if not search.isdigit():
            port_search_conditions.extend([
                PortV2.service_name.ilike(f'%{search}%'),
                PortV2.service_product.ilike(f'%{search}%')
            ])
        
        if port_search_conditions:
            search_port_subquery = db.query(Host.id).join(PortV2).filter(or_(*port_search_conditions))
            combined_search = or_(
                or_(*host_search_conditions),
                Host.id.in_(search_port_subquery)
            )
        else:
...
def get_host_filter_data_v2(db: Session = Depends(get_db)):
    ...
    common_ports = db.query(
        ...
        PortV2.service_name,
        PortV2.state,
        func.count(PortV2.id).label('count')
    ).group_by(
        PortV2.port_number,
        PortV2.service_name,
        PortV2.state
    ).order_by(
        func.count(PortV2.id).desc()
    ).limit(100).all()
    ...
    services = db.query(
        ...
        PortV2.service_name.isnot(None),
        PortV2.service_name != ''
    ).group_by(
        PortV2.service_name
    ).order_by(
    ...
    operating_systems = db.query(
        ...
        Host.os_name.isnot(None),
        Host.os_name != ''
    ).group_by(
        Host.os_name
    ).order_by(
        func.count(Host.id).desc()
    ).limit(20).all()
        ...
        try:
            ...
            if network.num_addresses <= 1000:
                ...
                for host_ip in network.hosts():
                    ip_conditions.append(Host.ip_address == str(host_ip))
                # Also include network and broadcast addresses
                ip_conditions.append(Host.ip_address == str(network.network_address))
                ip_conditions.append(Host.ip_address == str(network.broadcast_address))
                ...
                if network.prefixlen >= 24:  # /24 or smaller
                    ...
                    subnet_conditions.append(Host.ip_address.like(f'{prefix}.%'))
            ...
            subnet_conditions.append(Host.ip_address.like(f'{subnet_cidr}%'))
